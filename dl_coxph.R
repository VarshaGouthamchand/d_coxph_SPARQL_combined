...
call <- function(client, method, ...) {
  ...
  task = list(
    "name"="CoxPH",
    #"image"="varshagouthamchand/dl_coxph_combined",
    "image"="ananyac/dl_coxph",
    "collaboration_id"=client$get("collaboration_id"),
    "input"=input,
    "description"="",
    "organization_ids"=list(c(1))
  )
...
docker.wrapper <- function() {
  ...
  datauri <- readChar(database_uri,nchars=1e6)
  datauri <- trimws(datauri)
  print(datauri)
  source_python("run_sparql_for_cox.py")
  run_sparql_for_cox(datauri)
